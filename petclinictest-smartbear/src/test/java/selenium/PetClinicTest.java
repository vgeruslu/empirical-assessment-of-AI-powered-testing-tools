...
import com.smartbear.visualtest.models.TestRunResult;
...
public class PetClinicTest {
    ...
    public void testAddOwner() throws Throwable {
        ...
        VisualTest visualTest = new VisualTest(driver, "N9e5Mk9w/9QO5BxeroIE=");
        visualTest.capture("WelcomePage");

        // This line of code below is to test dynamic content.
        // We can imagine that the container in the welcome page can be a clinic update board, which may change daily
        // So when performing Visual tests we do not want these changes to be picked up by the AI, so by adding
        // this code we can ignore the dynamic content and only focus on actual visual changes made to the SUT

//        visualTest.capture("WelcomePage", new HashMap<String, Object>() {{
//            put("ignoreElements", new String[]{".container", "body > div.container-fluid > div > div.container"}); }});

        visualTest.printReport();
        TestRunResult result = visualTest.getTestRunResult();
        assert result.getFailed() == 0;
    ...
    public void testEditOwnerFirstName() throws Throwable {
        ...
        VisualTest visualTest = new VisualTest(driver, "N9e5Mk9w/9QO5BxeroIE=");
        visualTest.capture("EditOwnerPage");
        visualTest.printReport();
        TestRunResult result = visualTest.getTestRunResult();
        assert result.getFailed() == 0;
